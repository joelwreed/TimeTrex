...
/*
 * $Revision: 2490 $
 * $Id: FastTree.class.php 2490 2009-04-24 22:13:40Z ipso $
...
class FastTree {
	...
	function getAllChildren( $object_id = NULL, $recurse = FALSE ) {
		...
		$query  = '
				...
				LEFT JOIN '. $this->table .' b ON a.tree_id = b.tree_id AND a.left_id BETWEEN b.left_id AND b.right_id
				';
//				LEFT JOIN 	'. $this->table .' b ON b.left_id < a.left_id AND b.right_id > a.right_id
//				LEFT JOIN '. $this->table .' b ON a.left_id BETWEEN b.left_id AND b.right_id
//				LEFT JOIN '. $this->table .' b ON a.left_id BETWEEN b.left_id AND b.right_id
		switch (strtoupper($recurse)) {
			case 'RECURSE':
				...
				//Why is this here?
				//To exclude the parent?
				//SHould we ever do that?
				//Yes, but only when the passed object is forsure NULL!

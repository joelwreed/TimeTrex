...
class ProgressBar {
	...
	function start( $key, $total_iterations = 100, $update_iteration = NULL )  {
		Debug::text('start: '. $key .' Iterations: '. $total_iterations, __FILE__, __LINE__, __METHOD__,9);
	...
	function set( $key, $current_iteration ) {
		if ( $key == '' ) {
			return FALSE;
		}
		...
		if ( $current_iteration == 1 OR $current_iteration % $this->update_iteration == 0 ) {
 ...
 */
	...
	function get($key) {
		...
		Debug::text('get: '. $key, __FILE__, __LINE__, __METHOD__,9);

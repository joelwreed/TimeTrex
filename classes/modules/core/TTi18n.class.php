...
/*
 * $Revision: 3143 $
 * $Id: TTi18n.class.php 3143 2009-12-02 17:21:41Z ipso $
...
class TTi18n {
	...
	*/
	static public function getLanguageArray() {
		...
		$beta_langs = array( 'fr','de','it','pt', 'da' );

		$retarr2 = array();
		foreach( $supported_langs as $language ) {
			if ( in_array( $language, $beta_langs ) ) {
				$retarr2[$language] = $retarr[$language] .' (BETA)';
			} else {
	...
	static public function getText( $str ) {
		return self::getTranslationHandler()->get( $str );
	}

	static public function gt( $str ) {
	...
	*/
	static protected function _normalizeLocale($locale){
		...
		//Using .UTF-8 fails using Translation2 or Windows. Perhaps we attempt to add this later if it
	...
	*/
	...
	static public function formatNumber( $number, $auto_format_decimals = FALSE, $max_decimals = 4 ) {
 ...
 * @package Core
 *
 * Let's define a generic interface for a translation handler.

...
class PayrollDeduction_CA extends PayrollDeduction_CA_Data {
	...
	function setYearToDateCPPContribution($value) {
		/*
		if ($value > $this->cpp_employee_maximum_contribution) {
			$value = $this->cpp_employee_maximum_contribution;
		}
		*/
		if ( $value > 0 ) {
	...
	function setYearToDateEIContribution($value) {
		/*
		if ($value > $this->ei_employee_maximum_contribution) {
			$value = $this->ei_employee_maximum_contribution;
		}
		*/
		if ( $value > 0 ) {
	...
	function getFederalEmploymentCredit() {
		...
		*/
		...
		if ( $this->getDate() >= strtotime('01-Jan-09') ) {
			$tmp2_K4 = bcmul( $this->getData()->getFederalLowestRate(), 1044 );
		}elseif ( $this->getDate() >= strtotime('01-Jan-08') ) {
			$tmp2_K4 = bcmul( $this->getData()->getFederalLowestRate(), 1019 );
		} elseif ( $this->getDate() >= strtotime('01-Jan-07') ) {
			$tmp2_K4 = bcmul( $this->getData()->getFederalLowestRate(), 1000 );
		} else {
			$tmp2_K4 = bcmul( $this->getData()->getFederalLowestRate(), 500 );
		}

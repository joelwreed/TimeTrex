...
/*
 * $Revision: 3021 $
 * $Id: PayStubList.php 3021 2009-11-11 23:33:03Z ipso $
...
switch ($action) {
		...
		if ( is_array( $ids ) AND count($ids) > 0 ) {
			foreach ($ids as $id) {
				...
				foreach ($pslf as $pay_stub_obj) {
					...
					if ( is_object($pay_stub_obj->getPayPeriodObject())
							AND $pay_stub_obj->getPayPeriodObject()->getStatus() != 20 ) { //Closed
			...
			if ( is_array( $ids ) AND count($ids) > 0 ) {
				foreach ($ids as $id) {
					...
					foreach ($pslf as $pay_stub_obj) {
						...
						//if ( $pay_stub_obj->getStatus() == 10 ) {
						if ( is_object($pay_stub_obj->getPayPeriodObject())
								AND $pay_stub_obj->getPayPeriodObject()->getStatus() != 20 //Open/Adjustment
	...
	default:
		...
		$export_type_options = $pslf->getOptions('export_type');

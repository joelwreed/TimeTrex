...
/*
 * $Revision: 2759 $
 * $Id: GeneralLedgerSummary.php 2759 2009-08-28 18:52:56Z ipso $
 * $Date: 2009-08-28 11:52:56 -0700 (Fri, 28 Aug 2009) $
 */
								...
								if ( isset($psea_arr[$pse_obj->getPayStubEntryNameId()]) ) {
									if ( isset($psea_arr[$pse_obj->getPayStubEntryNameId()]['debit_account'])
										...
										foreach( $debit_accounts as $debit_account ) {
											...
											//Skip negative amounts completely and Don't include entries that are $0.
									...
									if ( isset($psea_arr[$pse_obj->getPayStubEntryNameId()]['credit_account'])
										...
										foreach( $credit_accounts as $credit_account) {
											...
											Debug::Text('Credit Entry: Account: '. $credit_account .' Amount: '. $pse_obj->getAmount() , __FILE__, __LINE__, __METHOD__,10);

											//Don't include entries that are $0.
											if ( $pse_obj->getAmount() > 0 ) {
												$raw_je_records[] = array(
																	'type' => 'credit',

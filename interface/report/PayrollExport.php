			...
			if ( isset($rows) ) {
				switch ( $filter_data['export_type'] ) {
					...
					case 'paychex_preview': //Paychex Preview export format.
						...
						break;
					case 'ceridian_insync': //Paychex Preview export format.
						$data = NULL;
						...
						break;
					case 'millenium': //Millenium export format.
						$data = NULL;
					...
					case 'quickbooks': //Quickbooks Pro export format.
						...
						foreach($rows as $row) {
							foreach( $setup_data['quickbooks']['columns'] as $column_id => $column_data ) {
								if ( isset( $row[$column_id] ) AND trim($column_data['hour_code']) != '' ) {
									$data .= 'TIMEACT	'. date('n/j/y', $row['pay_period_end_date']).'		'. $row['first_name'].'  '. $row['last_name'].'		'. trim($column_data['hour_code']) .'	'.  TTDate::getTimeUnit( $row[$column_id], 10 ) .'			Y	0'."\n";
								}
							}
						}

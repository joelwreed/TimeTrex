...
/*
 * $Revision: 2490 $
 * $Id: WagesPayableSummary.php 2490 2009-04-24 22:13:40Z ipso $
...
if ( $aplf->getRecordCount() > 0 ) {
	foreach ($aplf as $ap_obj ) {
		...
		if ( $ap_obj->getType() == 10 ) {
			$policy_rates['absence_policy-'.$ap_obj->getId()] = 1;
							...
							foreach($data_c as $date_stamp => $data_d) {
								...
								foreach($data_d as $column => $data_z) {
									foreach($data_z as $data_y) {
										...
										$uw_obj = getUserWageObject($user_wage_id);
										if ( is_object($uw_obj) ) {
											if ( $currency_convert_to_base == TRUE ) {
										...
										if ( isset($policy_rates[$column]) ) {
											if ( strpos( $column, 'premium') !== FALSE AND is_object($policy_rates[$column]) ) {
												//Debug::Text('Column: '. $column .' Premium Policy, has dynamic rate.', __FILE__, __LINE__, __METHOD__,10);

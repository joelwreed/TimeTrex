...
/*
 * $Revision: 3091 $
 * $Id: ViewUserTimeSheet.php 3091 2009-11-18 18:00:31Z ipso $
...
if ( isset($override_action) AND $override_action != '') {
...
switch ($action) {
	...
	case 'verify':
		...
		if ( $pptsvlf->getRecordCount() == 0 ) {
			...
			Debug::text('Timesheet re-verified by employee...', __FILE__, __LINE__, __METHOD__,10);
		...
		//With the new hierarchy system there will never be a case where the timesheet verification can be immiedately authorized
		//as we can only determine hierarchies based on subordinates now.
		$pptsvf->setStatus(30); //Pending Auth
	...
	case 'calculate adjustment':
		...
		//Redirect::Page( URLBuilder::getURL( array('filter_user_id' => $filter_data['user_id'], 'filter_date' => $filter_date ), '../timesheet/ViewUserTimeSheet.php') );
		Redirect::Page( URLBuilder::getURL( array('user_id' => $filter_data['user_id'] ), '../pay_stub_amendment/PayStubAmendmentList.php') );

		break;
	case 'recalculate company':
...
*/
		...
		*/
		...
		$smarty->assign_by_ref('is_previous_time_sheet_verified', $is_previous_time_sheet_verified);
		...
		$action_options = array(
							...
							'recalculate employee' => 	TTi18n::gettext('Recalculate Employee'),
							'recalculate company' => 	TTi18n::gettext('Recalculate Company') );

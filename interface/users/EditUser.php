...
/*
 * $Revision: 2684 $
 * $Id: EditUser.php 2684 2009-07-24 22:47:23Z ipso $
 * $Date: 2009-07-24 15:47:23 -0700 (Fri, 24 Jul 2009) $
 */
	...
	case 'submit':
		...
		if ( DEMO_MODE == FALSE OR $uf->isNew() == TRUE ) {
			if ( !empty($user_data['id']) ) {
				...
				if ( !empty($user_data['phone_password']) OR !empty($user_data['phone_password2']) ) {
					if ( $user_data['phone_password'] == $user_data['phone_password2'] ) {
						$uf->setPhonePassword($user_data['phone_password']);
					} else {
						$uf->Validator->isTrue( 'phone_password',
												FALSE,
												TTi18n::gettext('Passwords don\'t match') );
					}
				}
		...
		if ( DEMO_MODE == FALSE AND isset($user_data['permission_control_id']) AND ( $permission->Check('permission','edit') OR $permission->Check('permission','edit_own') OR $permission->Check('user','edit_permission_group') ) ) {

...
{/literal}
	...
	{if $permission->Check('wage','enabled') AND $permission->Check('wage','view')}
		aI("text={t}Wage Groups{/t};url={$BASE_URL}company/WageGroupList.php;");
	{/if}
	{if $permission->Check('station','enabled') AND $permission->Check('station','view')}
...
{if $permission->Check('user','edit_own')}
	...
	aI("text={t}Phone Password{/t};url={$BASE_URL}users/EditUserPhonePassword.php?id={$current_user->getId()};");
{/if}
...
{/if}
aI("text={t}Administrator Guide{/t};target=new;url=http://www.timetrex.com/wiki/index.php/TimeTrex_{php}echo getTTProductEditionName(){/php}_Edition_Administrator_Guide_v{$system_settings.system_version};");
aI("text={t}FAQ{/t};target=new;url=http://www.timetrex.com/wiki/index.php/TimeTrex_{php}echo getTTProductEditionName(){/php}_Edition_FAQ_v{$system_settings.system_version};");

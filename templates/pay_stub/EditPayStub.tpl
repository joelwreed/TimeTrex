...
function editPayStubAlert() {
	var alertMsg= "{/literal}{t}WARNING: Manually editing pay stubs can result in incorrect amounts being calculated for things such as taxes and vacation accrual.\nPlease use Pay Stub Amendments instead.\nDO NOT EDIT PAY STUBS MANUALLY UNLESS ABSOLUTELY NECCESARY!{/t}{literal}";
	alert(alertMsg);
}
			...
			{foreach name=other from=$data.entries.50 item=pay_stub_entry}
				...
				<tr class="{$row_class}" align="right">
					...
					<td>
						{if $pay_stub_entry.type == 40}<b>{/if}
						{$pay_stub_entry.ytd_amount}
						<input type="hidden" name="data[entries][{$pay_stub_entry.type}][{$pay_stub_entry.id}][ytd_amount]" value="{$pay_stub_entry.ytd_amount}">
						<input type="hidden" name="data[entries][{$pay_stub_entry.tmp_type}][{$pay_stub_entry.id}][id]" value="{$pay_stub_entry.id}">
						<input type="hidden" name="data[entries][{$pay_stub_entry.tmp_type}][{$pay_stub_entry.id}][tmp_type]" value="{$pay_stub_entry.tmp_type}">
						{if $pay_stub_entry.type == 40}</b>{/if}
					</td>

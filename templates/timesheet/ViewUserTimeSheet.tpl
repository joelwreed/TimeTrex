		...
		<form name="timesheet" method="get" action="{$smarty.server.SCRIPT_NAME}">
				...
				<tr valign="top">
					<td colspan="2">
						...
						{if $permission->Check('punch','verify_time_sheet')}
							{if $is_previous_time_sheet_verified == FALSE
								AND $current_time <= $pay_period_verify_window_end_date
								AND is_object( $current_user ) AND is_object( $user_obj )
								AND $current_user->getId() == $user_obj->getId()}
							<table class="tblList">
								<tr class="tblDataWarning">
									<td colspan="3">
										<b>{t}Previous Pay Period requires verification!{/t}</b>
									</td>
								</tr>
							</table>
							{/if}
							...
							<table class="tblList">
								...
								{if $time_sheet_verify != ''}
									<tr class="tblDataWhiteNH">
										<td>
											{$user_obj->getFullName()}
										</td>
										<td>
											{getdate type="DATE+TIME" epoch=$time_sheet_verify.created_date}
										</td>
									</tr>
									<tr class="tblDataWhiteNH">
										<td colspan="2" {if $time_sheet_verify.status_id == 55}bgcolor="red"{/if}>
											{$time_sheet_verify.status}
											{if $time_sheet_verify.status_id == 55
												AND $current_time >= $pay_period_verify_window_start_date
												AND $current_time <= $pay_period_verify_window_end_date}
												<input type="SUBMIT" class="button" name="override_action" value="{t}Verify{/t}" onClick="return confirmSubmit('{t}By pressing OK, I hereby certify that my timesheet for the pay period of{/t} {getdate type="DATE" epoch=$pay_period_start_date} {t}to{/t} {getdate type="DATE" epoch=$pay_period_end_date} {t}is accurate and correct.{/t}');">
											{/if}
										</td>
									</tr>
						...
						<table class="tblList">
							...
							<tr>
								<td colspan="2" align="center">
									...
									<input type="SUBMIT" class="button" value="{t}Submit{/t}" onClick="return confirmAction();">
								</td>
							</tr>
							{/if}
						</table>
